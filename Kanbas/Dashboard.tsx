...
import { fetchEnrolledCourses, enrollCourse, unenrollCourse } from "./client";
...
export default function Dashboard({
  ...
  useEffect(() => {
    const loadEnrollments = async () => {
      if (currentUser) {
        try {
          const enrolledCoursesData: CourseTypes[] = await fetchEnrolledCourses(
            currentUser._id
          );
          dispatch(setEnrollments(enrolledCoursesData.map((c) => c._id))); // Sync Redux with backend
        } catch (error) {
